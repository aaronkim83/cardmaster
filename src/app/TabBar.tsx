...
function Tab({ tab, active, onClick }: { tab: { id: ScreenId; icon: string; label: string }; active: boolean; onClick: () => void }) {
  return (
    <button data-testid={`tab-${tab.id}`} onClick={onClick} className={`flex-1 pt-1.5 text-center text-[10px] font-semibold ${active ? 'text-ink' : 'text-faint'}`}>
